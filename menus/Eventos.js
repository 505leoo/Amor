...
const Eventos = memo(({ navigation, temporada, soloEvento = false }) => {
  ...
  const navegarAlEvento = () => eventoActual && navigation?.navigate(eventoActual.destino, { from: 'main' });
  return <View style={[styles.eventoWrap, styles.eventoWrapAjuste]}>
    <View style={styles.eventoBtn} onStartShouldSetResponder={() => true} onResponderGrant={({ nativeEvent }) => { eventoSwipeStart.current = nativeEvent.pageX; }} onResponderRelease={({ nativeEvent }) => { const distancia = nativeEvent.pageX - eventoSwipeStart.current; if (Math.abs(distancia) > 25 && eventos.length > 1) setEventoActivo(actual => (actual + 1) % eventos.length); else navegarAlEvento(); eventoSwipeStart.current = null; }} accessibilityRole="button" accessibilityLabel={`Ver evento ${eventoActual?.titulo || ''}`}>
