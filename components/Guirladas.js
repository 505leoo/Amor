...
const generateBulbs = () => Array.from({ length: BULB_COUNT }).map((_, i) => {
...
const makePath = () => {
  const pts = Array.from({ length: 80 }).map((_, i) => {
    const t = i / 79;
    return { x: START_X + t * (END_X - START_X), y: getRopeY(t) };
  });
...
const BulbGlow = memo(({ x, bulbY, r, delay, minGlow, maxGlow, isPaused }) => {
  ...
  }, [opacity, delay, minGlow, maxGlow, isPaused]);
  ...
  return prev.x === next.x && prev.bulbY === next.bulbY && prev.r === next.r && prev.isPaused === next.isPaused;
});
...
), () => true); // Nunca re-renderizar este componente
